...
import { getApprovedQuestions, createQuestion, deleteQuestion } from '../api/question';
import { useSelector } from 'react-redux';

const { TextArea } = Input;
const { Option } = Select;

const categories = ['全部', '雕刻技艺', '材料处理', '作品鉴赏', '文化传承', '学习交流', '其他'];
...
const QuestionPage = () => {
  ...
  const fetchQuestions = (category) => {
    setLoading(true);
    getApprovedQuestions(category === '全部' ? '' : category)
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <Tag color={categoryColors[item.category] || 'default'}>{item.category || '其他'}</Tag>
                  {user && item.author?._id === user._id && (
                    <Popconfirm
                      title="确认删除"
                      description="确定要删除这个问题吗？相关评论也会被一并删除。"
                      onConfirm={(e) => handleDeleteQuestion(item._id, e)}
                      okText="删除"
                      cancelText="取消"
